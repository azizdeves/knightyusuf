...
public class TaskSeedServlet extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		if(req.getParameter("k")!=null){
			...
			stat.setKey("loooooool");
			stat.setSerializedData("seriel");
			dao.saveStat(stat);
...
//		        Set<String> attributes = new HashSet<String>();
	...
	public void stat(String name, String momin, Date week, long time){
		TaskDao dao = new TaskDao();
		Statistic stat=null;
		{
			Statistic stats[] = dao.getStatisitc(name, momin , week,1);
			if(stats.length>0)
				stat = stats[0]; 
		}
		if(stat != null)
...
//			week = TaskServiceImpl.getStartWeek(week);
			Task[] tsks = dao.getWeekTasksStat(name, momin, week);
